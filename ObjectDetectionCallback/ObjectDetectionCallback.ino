...
void ODPostProcess(std::vector<ObjectDetectionResult> results)
{
    ...
    int productCount = 0; //目前已經發現的物件類型數量

    if (ObjDet.getResultCount() > 0) {
        for (int i = 0; i < ObjDet.getResultCount(); i++) {
            int obj_type = results[i].type();
            if (itemList[obj_type].filter) {    // check if item should be ignored
                if(obj_type == 0) productCount++;
                if(obj_type == 1) productCount++;
                if(obj_type == 3) productCount++;
    ...
    //印出目前物件數量(no 中文 或 全形字)
    String ShowText = "ProductCount:" + String(productCount);
    Serial.println(ShowText);
    OSD.drawText(CHANNEL, 100, 100, ShowText.c_str(), OSD_COLOR_RED);
    OSD.update(CHANNEL);
}
